...
export async function sendEnquiryNotification(data: { name: string, email: string, phone?: string, type: string, message: string }) {
  if (!resend) {
...
export async function sendEnquiryConfirmation(data: { name: string, email: string, type: string }) {
  if (!resend) return;
  try {
    await resend.emails.send({
      from: emailFrom,

...
export default function ChatAdminClient({
  ...
  useEffect(() => {
    ...
    async function pollConversations() {
      try {
        // Rather than querying another endpoint, we can query our messages endpoint for the selected chat,
        // and for the list, we can use the separate active_list polling effect.
        // If selectedId is set, we poll for the messages of that selected conversation.
        if (selectedId) {
          const mRes = await fetch(`/api/chat/messages?conversationId=${selectedId}`);
          if (mRes.ok) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(pollList, 4000); // list can poll slower (4s) to save performance
    return () => {
      active = false;
      clearInterval(timer);
    };
  ...
  const handleTakeOver = async () => {
    ...
    try {
      await takeOverConversation(selectedId, adminEmail);
  ...
  const handlePause = async () => {
    ...
    setIsSubmitting(true);
    try {
      await pauseConversation(selectedId);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const msg = await addChatMessage(selectedId, "ADMIN", messageText);

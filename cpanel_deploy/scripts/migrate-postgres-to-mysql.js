...
async function main() {
    ...
    const pg = new pg_1.Pool({ connectionString: postgresUrl });
    ...
    try {
        ...
        for (const table of tables) {
            ...
            const sourceResult = await pg.query(`SELECT ${pgColumns} FROM ${quotePgIdentifier("public")}.${quotePgIdentifier(table.name)}`);
            const [[beforeCountRow]] = await mysqlPool.query(`SELECT COUNT(*) AS count FROM ${quoteMysqlIdentifier(table.name)}`);
            ...
            if (sourceResult.rows.length > 0) {
                ...
                for (const row of sourceResult.rows) {
                    ...
                    await mysqlPool.query(`INSERT INTO ${quoteMysqlIdentifier(table.name)} (${mysqlColumns}) VALUES (${placeholders}) ON DUPLICATE KEY UPDATE ${noopUpdate}`, values);
                    insertedOrUpdated += 1;
                }
            }
            const [[afterCountRow]] = await mysqlPool.query(`SELECT COUNT(*) AS count FROM ${quoteMysqlIdentifier(table.name)}`);
            results[table.name] = {

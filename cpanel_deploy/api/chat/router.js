...
function isWorkflowExpired(workflowUpdatedAt) {
...
function classifyIntent(text) {
    ...
    if (/(where|how|can|could|want|need|should).*(upload|send|attach|submit|put).*(cv|resume|curriculum)/i.test(lower) ||
    ...
    if (/(where\s+should\s+i\s+send\s+it|how\s+to\s+send\s+it|can\s+i\s+send\s+it)/i.test(lower) ||
...
function quickReplies(text, buttons) {
    return {
        intent: 'UNKNOWN',
        confidence: 0.9,
        response: text,
...
// ─── Main Router ─────────────────────────────────────────────────────────────
async function processChatIntent(text, workflow) {
    ...
    if (intent === 'CANDIDATE_CV_UPLOAD') {
        ...
        return {
            ...
            cardType: 'CV_UPLOAD',
            data: { link: tools_1.APPROVED_WEBSITE_LINKS.UPLOAD_CV, buttons: ['📄 Upload CV Here', '🌐 Open Upload Page', '❌ I Don\'t Have a CV'] },
    ...
    if (intent === 'JOB_SEARCH') {
        ...
        const jobs = await (0, tools_1.searchJobs)({ query, location });
        ...
        if (jobs.length === 0) {
            return {
                ...
                cardType: 'QUICK_REPLIES',
                data: { buttons: ['📄 Upload Your CV', '🌐 View All Jobs', '🎯 Tell Us Preferred Role'], link: tools_1.APPROVED_WEBSITE_LINKS.UPLOAD_CV },
                nextWorkflowType: 'NONE',
                nextWorkflowState: 'IDLE',
            };
        }
        const jobList = jobs.slice(0, 5).map((j) => `• **[${j.title}](/jobs/${j.id})** — ${j.type}\n  📍 ${j.location}`).join('\n\n');
        return {
            intent: 'JOB_SEARCH',
            confidence,
...
// ─── Employer Workflow ────────────────────────────────────────────────────────
async function handleEmployerStep(text, workflow) {
...
// ─── Candidate Workflow ───────────────────────────────────────────────────────
async function handleCandidateStep(text, workflow) {
    const val = sanitiseText(text);
    const { workflowState } = workflow;
    const data = workflow.workflowData || {};
    ...
    switch (workflowState) {
        ...
        case 'CANDIDATE_CONFIRMATION': {
            ...
            if (/^(yes|confirm|submit|send|ok|okay|go ahead|proceed|sure|correct|right)/.test(lower)) {
                try {
                    const { candidate: cRecord, isNew } = await (0, tools_1.createOrUpdateCandidate)({
                        name: data.name,
                        email: data.email,
                        phone: data.phone,
                    ...
                    const { candidate: candTable } = await Promise.resolve().then(() => __importStar(require('../../db/schema')));
                    ...
                    if (data.selectedJobId) {
                        const { createJobApplication } = await Promise.resolve().then(() => __importStar(require('./tools')));
...
function generateCandidateConfirmation(data) {
    const summary = [
        `**Name:** ${data.name || '—'}`,
        `**Email:** ${data.email || '—'}`,
        `**Phone:** ${data.phone || '—'}`,

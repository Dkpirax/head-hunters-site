...
exports.adminUsersRouter.get('/', async (req, res) => {
    try {
        const users = await db_1.db.select().from(schema_1.adminUser).orderBy((0, drizzle_orm_1.desc)(schema_1.adminUser.createdAt));
        const allPermissions = await db_1.db.select().from(schema_1.permission);
        const userPerms = await db_1.db.select({
            userId: schema_1.userPermission.userId,
            permissionName: schema_1.permission.name,
        })
            .from(schema_1.userPermission)
            .innerJoin(schema_1.permission, (0, drizzle_orm_1.eq)(schema_1.userPermission.permissionId, schema_1.permission.id));

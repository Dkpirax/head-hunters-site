...
exports.knowledgeRouter.post('/reindex', async (req, res) => {
    ...
    try {
        ...
        await db_1.db.update(schema_1.knowledgeDocument)
            .set({ status: 'FAILED' })
            .where((0, drizzle_orm_1.eq)(schema_1.knowledgeDocument.version, version));
        res.status(500).json({ error: error.message });
    }
});
...
exports.knowledgeRouter.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [doc] = await db_1.db.select().from(schema_1.knowledgeDocument).where((0, drizzle_orm_1.eq)(schema_1.knowledgeDocument.id, id)).limit(1);
        if (!doc)
            return res.status(404).json({ error: 'Document not found' });
        ...
        await db_1.db.delete(schema_1.knowledgeChunk).where((0, drizzle_orm_1.eq)(schema_1.knowledgeChunk.documentId, id));
        await db_1.db.delete(schema_1.knowledgeDocument).where((0, drizzle_orm_1.eq)(schema_1.knowledgeDocument.id, id));

...
export function Hero({ settings }: { settings: any }) {
  ...
  useEffect(() => {
    if (typeof window === "undefined" || window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      ...
      gsap.set(".hh-header-anim, .hh-word, .hh-lede, .hh-cta, .hh-panel, .hh-journey-card", { opacity: 1, y: 0, scale: 1 });
      return;
    }

    const ctx = gsap.context(() => {
      // Set initial state to avoid FOUC
      gsap.set(".hh-header-anim, .hh-word, .hh-lede, .hh-cta, .hh-panel", { opacity: 0 });
      gsap.set(".hh-word", { y: 70 });
      gsap.set(".hh-lede", { y: 30 });
      gsap.set(".hh-cta", { y: 24 });
      gsap.set(".hh-panel", { y: 36, scale: 0.97 });
      gsap.set(".hh-header-anim", { y: -20 });

      const tl = gsap.timeline({ defaults: { ease: "power4.out" } });

      tl.to(".hh-header-anim", { y: 0, opacity: 1, duration: 0.7 })
        .to(".hh-word", { y: 0, opacity: 1, duration: 1.1, stagger: 0.08 }, "-=0.2")

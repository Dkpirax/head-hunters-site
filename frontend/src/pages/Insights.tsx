          ...
          <div className="max-w-[1200px] mx-auto px-5">
            {loading ? (
              <div className="py-20 flex justify-center">
                <div className="w-8 h-8 border-2 border-[#04a891] border-t-transparent rounded-full animate-spin"/>
              </div>
            ) : (
              <>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {articles.length === 0 ? (
                    ...
                    articles.map((article: any) => {
                      ...
                      return (
                        <Link to={`/insights/${article.slug}`} key={article.id} className="block group">

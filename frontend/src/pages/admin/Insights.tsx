  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(
        isNew ? "/api/admin/insights" : `/api/admin/insights/${editing.id}`,
        {
  ...
  return (
    <div className="p-8 h-[calc(100vh-4rem)] flex flex-col overflow-hidden">
      <div className="flex items-center justify-between mb-8 shrink-0">
        ...
        <button
          onClick={openNew}
          className="inline-flex items-center gap-2 h-10 px-5 rounded-[10px] bg-[#02695e] hover:bg-[#027d6f] text-white text-sm font-bold transition-all shadow-[0_4px_12px_rgba(2,105,94,0.2)] cursor-pointer"
        >
          ...
          <div className="w-full max-w-[800px] bg-[#111413] border border-white/10 rounded-[20px] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.5)] my-auto mt-[10vh]">
            ...
            <div className="flex gap-3 mt-6 pt-5 border-t border-white/6">
              <button
                onClick={save}
                disabled={saving}
                className="inline-flex items-center justify-center gap-2 h-10 px-5 rounded-[10px] bg-[#02695e] hover:bg-[#027d6f] text-white text-sm font-bold transition-all disabled:opacity-50 cursor-pointer"
              >
                <CheckCircle size={15} /> {saving ? "Saving..." : (isNew ? "Create Article" : "Save changes")}
              </button>
              <button
                onClick={() => setEditing(null)}
                className="h-10 px-5 rounded-[10px] border border-white/10 bg-white/5 text-white/60 text-sm font-semibold hover:text-white hover:border-white/20 transition-all cursor-pointer"
              >
                Cancel
              </button>
            </div>

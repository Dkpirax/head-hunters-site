...
export function AdminInsightsPage() {
  ...
  const fetchArticles = async () => {
    try {
      const data = await apiClient("/api/admin/articles");
      setArticles(data);
    } catch (err) {
  ...
  const save = async () => {
    ...
    try {
      if (isNew) {
        await apiClient("/api/admin/articles", {
          method: "POST",
          body: JSON.stringify({
            title: editing.title,
            slug: editing.slug,
        ...
        await apiClient(`/api/admin/articles/${editing.id}`, {
          method: "PUT",
          body: JSON.stringify({
            title: editing.title,
            slug: editing.slug,
  ...
  const remove = async (id: string) => {
    try {
      await apiClient(`/api/admin/articles/${id}`, { method: "DELETE" });
      await fetchArticles();
      setDeletingId(null);
    } catch (err: any) {
  ...
  if (loading) {
    return <div className="p-8 text-white">Loading insights...</div>;
  }
                ...
                <div>
                  ...
                  <input id="art-title" value={editing.title}
                    onChange={(e) => {
                      ...
                      let shouldUpdateSlug = isNew && (editing.title === editing.slug.replace(/-/g, ' '));
                      if (isNew && editing.slug === "") shouldUpdateSlug = true; // Auto fill slug when empty
                      setEditing({ ...editing, title: e.target.value, slug: shouldUpdateSlug ? newSlug : editing.slug });
                    }}

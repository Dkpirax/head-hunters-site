...
export function AdminEnquiriesPage() {
  ...
  const loadEnquiries = useCallback(async (isPoll = false) => {
    ...
    try {
      const data = await apiClient("/api/admin/enquiries");
      setEnquiries((prev) => {
  ...
  const updateStatus = async (id: string, status: EnquiryStatus) => {
    try {
      await apiClient(`/api/admin/enquiries/${id}/status`, {
        method: "PUT",
  ...
  const markRead = (id: string) => updateStatus(id, "READ");
  const archive = (id: string) => updateStatus(id, "ARCHIVED");
  const unarchive = (id: string) => updateStatus(id, "READ");

  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to permanently delete this enquiry?")) return;
    try {
      await apiClient(`/api/admin/enquiries/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="p-8 h-[calc(100vh-64px)] flex flex-col">
      ...
      <AnimatePresence>
        {hasNewNotification && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="mb-5 p-4 rounded-[12px] bg-gradient-to-r from-[#02695e]/20 to-[#04a891]/20 border border-[#04a891]/30 flex items-center justify-between text-white text-sm shrink-0"
          >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white/3 border border-white/8 rounded-[16px] text-white/40 flex-1 min-h-0">
